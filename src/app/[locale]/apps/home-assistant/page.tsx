...
export async function generateMetadata({
  ...
  return { title: dict.meta.titleHomeAssistant, description: dict.homeAssistantPage.hero.subtitle };
}
...
export default async function HomeAssistantPage({
  ...
  const locale: Locale = isLocale(raw) ? raw : "en";
  const dict = getDictionary(locale);
  const t = dict.homeAssistantPage;
  const c = dict.common;
      ...
      <section className="border-b border-hairline">
        <div className="mx-auto max-w-3xl px-5 py-20 sm:px-8">
          <p className="font-mono text-[12px] tracking-[0.14em] text-slate uppercase">
            {t.openSection.eyebrow}
          </p>
          <h2 className="font-display mt-3 text-3xl font-bold text-platinum sm:text-4xl">
            {t.openSection.title}
          </h2>
          <div className="mt-8 space-y-5 text-chrome-dim">
            {t.openSection.paragraphs.map((p, i) => (
              <p key={i}>{p}</p>
            ))}
          </div>
        </div>
        <div className="mx-auto max-w-6xl px-5 pb-20 sm:px-8">
          <div className="grid grid-cols-1 gap-px overflow-hidden bg-hairline sm:grid-cols-2 lg:grid-cols-4">
            {t.openSection.items.map((item) => (
              <div key={item.name} className="bg-void p-6">
                <p className="font-semibold text-platinum">{item.name}</p>
                <p className="mt-2 text-sm text-chrome-dim">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
