 ...
 * and only appears once the visitor has scrolled past the hero, so it never
...
export function WhatsappCta({ locale }: { locale: Locale }) {
  ...
  const message = ar
    ? "السلام عليكم، مهتم بأنظمة سيلترا للمنزل الذكي وأرغب في معرفة التفاصيل."
    : "Hello, I'm interested in Syltra smart home systems and would like more details.";
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className={`fixed bottom-24 start-5 z-50 flex h-13 w-13 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/40 transition-all duration-300 hover:scale-105 sm:bottom-5 sm:h-14 sm:w-14 ${
